...
impl GeographicPathBuilder {
    ...
    pub async fn build_diverse_path(&self, target: NodeId, hops: u32, candidates: &[NetworkNode]) -> Result<Vec<NodeId>, anyhow::Error> {
        ...
        let initial_candidates = if !preferred_candidates.is_empty() {
        ...
        // Select first node
        if let Some(first_node) = initial_candidates.first() {
        ...
        // Select remaining nodes with maximum geographic diversity
        while selected_nodes.len() < hops as usize {
            ...
            for candidate in &located_candidates {
                ...
                if let Some(candidate_location) = candidate.location {
                    ...
                    for existing_location in &selected_locations {
                        let distance = self.location_service.calculate_distance(candidate_location, *existing_location);
                        min_distance = min_distance.min(distance);
                    }
                    
                    // Prefer nodes that are far from existing ones
                    if min_distance > best_diversity_score && min_distance >= self.diversity_config.min_distance_km {
